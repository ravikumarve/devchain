 ...
 * - Cloud mode: creates users in Supabase Auth first, then profiles in public.users.
 * - Local mode (no Supabase creds): creates users directly in the DB (bcrypt).
 *
 * Run: node seed.js  (or: npm run seed:demo from the monorepo root)
 */
require('dotenv').config({ path: __dirname + '/.env' });
const crypto = require('crypto');
const bcrypt = require('bcryptjs');
const prisma = require('./src/config/database');
const { adminClient: supabase, isCloudAuth } = require('./src/config/supabase');
const { serializeArray } = require('./src/utils/dbCompat');
...
async function upsertDemoUser({ email, username, bio, reputationScore }) {
  const isLocal = !isCloudAuth;
  let authUserId;

  if (isLocal) {
    // ── LOCAL MODE: no Supabase Auth — user lives entirely in the DB ──
    const existing = await prisma.user.findUnique({ where: { email } });
    authUserId = existing?.id || crypto.randomUUID();
    console.log(`  ℹ️  Local mode — user ${email} handled in DB`);
  } else {
    // ── CLOUD MODE: ensure user exists in Supabase Auth ──
  ...
  const user = await prisma.user.upsert({
    ...
    create: {
      ...
      reputationScore,
      isEmailVerified: true,
      // Local mode stores the demo password (bcrypt); cloud mode uses Supabase Auth
      ...(isLocal ? { passwordHash: await bcrypt.hash(DEMO_PASSWORD, 10) } : {}),
    },
  });
  ...
  const products = [
    {
      ...
      price: 2900, category: 'ui-kits', tags: serializeArray(['react', 'typescript', 'tailwind', 'accessible']),
      sellerId: seller.id,
    },
    {
      ...
      price: 4900, category: 'python-scripts', tags: serializeArray(['python', 'machine-learning', 'pipeline', 'data-science']),
      sellerId: seller.id,
    },
    {
      ...
      price: 3900, category: 'mobile-templates', tags: serializeArray(['react-native', 'ecommerce', 'stripe', 'mobile']),
      sellerId: seller.id,
    },
    {
      ...
      price: 14900, category: 'blockchain', tags: serializeArray(['solidity', 'ethereum', 'nft', 'hardhat']),
      sellerId: seller.id,
    },
    {
      ...
      price: 1900, category: 'node-packages', tags: serializeArray(['node', 'express', 'prisma', 'api']),
      sellerId: seller.id,
    },
    {
      ...
      price: 3400, category: 'react-components', tags: serializeArray(['react', 'tailwind', 'dashboard', 'components']),
      sellerId: seller.id,
    },
    {
      ...
      price: 2400, category: 'apis', tags: serializeArray(['openai', 'typescript', 'api', 'sdk']),
      sellerId: seller.id,
    },
    {
      ...
      price: 1500, category: 'tools', tags: serializeArray(['github-actions', 'ci-cd', 'devops', 'workflows']),
      sellerId: seller.id,
    },
  ];
  ...
  const jobs = [
    {
      ...
      skillsRequired: serializeArray(['React', 'TypeScript', 'ethers.js', 'Web3', 'DeFi']),
    ...
    },
    {
      ...
      skillsRequired: serializeArray(['Rust', 'Solidity', 'Security', 'DeFi', 'Auditing']),
    ...
    },
    {
      ...
      skillsRequired: serializeArray(['Python', 'FastAPI', 'Next.js', 'PostgreSQL', 'WebSocket']),
    ...
    },
    {
      ...
      skillsRequired: serializeArray(['React Native', 'TypeScript', 'UI/UX', 'Animations', 'Fintech']),
    ...
    },
    {
      ...
      skillsRequired: serializeArray(['Go', 'Kafka', 'Redis', 'PostgreSQL', 'Microservices']),
    ...
    },
    {
      ...
      skillsRequired: serializeArray(['Kubernetes', 'AWS', 'Terraform', 'Helm', 'ArgoCD']),
