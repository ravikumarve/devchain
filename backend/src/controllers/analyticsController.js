...
function generateInsights(products, stats) {
  ...
  if (stats.totalSalesThisMonth === 1 && stats.totalRevenueThisMonth > 0) {
    insights.push({
      type: 'success', icon: '🎉', title: 'First Sale This Month!',
  ...
  if (staleProducts.length > 0) {
    insights.push({
      ...
      action: 'Review Products', actionUrl: '/profile', priority: 'high',
    });
  }

  if (stats.totalProducts < 3 && stats.totalProducts > 0) {
    insights.push({
      type: 'tip', icon: '💡', title: 'Expand Your Catalog',
      description: 'Sellers with 3+ products earn 3x more on average. Consider listing another product.',
      action: 'Add Product', actionUrl: '/sell', priority: 'medium',
    });
  }

  if (stats.totalProducts === 0) {
    insights.push({
      type: 'tip', icon: '🚀', title: 'Start Selling Today',
      description: 'List your first product and join thousands of developers earning on DevChain.',
      action: 'Create Product', actionUrl: '/sell', priority: 'high',
    });
  }

  if (hotProducts.length > 0) {
    insights.push({
      type: 'success', icon: '🔥', title: 'Hot Products!',
...
async function calculateRanking(sellerId, sellerRevenue) {
  try {
    ...
    return { percentile, tier, rank: sellerRank + 1, totalSellers: allSellers.length };
  } catch (err) {
    log.error({ err }, 'Ranking calculation failed');
    return { percentile: 50, tier: 'Seller', rank: 1, totalSellers: 1 };
  }
}
